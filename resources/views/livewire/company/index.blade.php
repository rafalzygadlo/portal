<div class="col-12 px-md-3" x-data="{ view: 'grid' }">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-stretch align-items-sm-center mb-3">
        <h2 class="mb-1 fw-black text-center">Company</h2>
                    ...
                    @endif
                        ...
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <h5 class="fw-bold mb-0 me-3">
                                <a href="{{ route('company.show', $company) }}" class="text-dark text-decoration-none stretched-link">{{ $company->name }}</a>
                            </h5>
                        </div>

                        <p class="text-muted small mb-3">{{ Str::limit($company->description, 100) }}</p>
                        ...
                        <div class="mt-auto d-flex justify-content-between align-items-center pt-2 border-top">
                            <div class="small text-muted">
                                <i class="bi bi-clock-history"></i> {{ $company->created_at->diffForHumans() }}
                            </div>
                            <div class="d-flex align-items-center gap-2">
                                @if($company->subdomain)
                                    <a style="position: relative; z-index: 1;" href="https://{{ $company->subdomain }}.{{ env('DOMAIN_NAME') }}" target="_blank" class="btn btn-sm btn-light border-0" title="{{ $company->subdomain }}.{{ env('DOMAIN_NAME') }}"><i class="bi bi-globe"></i></a>
                                @endif
                                <livewire:favorite :model="$company" :key="'favorite-company-list-'.$company->id" />
                            </div>
                        </div>
                    </div>
                </div>
