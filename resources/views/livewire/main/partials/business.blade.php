...
<div
    class="card h-100 shadow-sm border-1 rounded-4 overflow-hidden flex-row flex-lg-column transition-all hover-shadow">

    <!-- MINIATURKA / ZDJĘCIE -->
    <a href="" class="position-relative d-block flex-shrink-0 col-4 col-lg-12 bg-light overflow-hidden"
        ...
        @if($item->images->isNotEmpty())
            <img loading="lazy" src="{{ asset('storage/' . $item->images->skip(0)->first()->path) }}"
                class="justify-content-center align-items-center object-fit-cover w-100 h-100" alt="{{ $item->title }}">
        @else
            <div
        ...
        <p class="card-text text-secondary small flex-grow-1 mb-2 d-none d-sm-block line-clamp-2"
            ...
            @if($item->subdomain)
                        <div class="mb-0 mt-auto pt-2 border-top border-light small text-secondary">
                            <a href="https://{{ $item->subdomain }}.{{ env('DOMAIN_NAME') }}" target="_blank" class="text-decoration-none text-primary fw-medium d-block text-truncate">
