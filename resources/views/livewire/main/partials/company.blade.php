<div class="card h-100 bg-white border-0 rounded-4 overflow-hidden transition-hover shadow-sm d-flex flex-column position-relative{{ ! empty($isPromoted) ? ' border border-warning shadow-lg' : '' }}">
    ...
    <a href="{{ route('company.show', $item) }}" class="d-block flex-shrink-0 bg-light overflow-hidden" 
    ...
    <div class="card-body p-3 p-lg-4 d-flex flex-column col-8 col-lg-12">
        <h6 class="card-title fw-bold mb-2 flex-grow-0">
            <a href="{{ route('company.show', $item)}}" class="text-decoration-none text-dark line-clamp-2 stretched-link hover-primary">
                {{ $item->name }}
        ...
        <div class="mt-2 d-flex justify-content-end">
            <livewire:favorite :model="$item" :key="'favorite-main-company-'.$item->id" />
        </div>
    </div>
</div>