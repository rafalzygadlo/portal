                ...
                <div class="modal-content shadow-lg">
                    ...
                    <div class="modal-body">
                        <form wire:submit.prevent="save">
                            ...
                            @if ($type === 'person')
                                <div class="mb-3">
                                    <label for="assigned-user-id" class="form-label">Assigned worker (optional)</label>
                                    <select id="assigned-user-id" wire:model="assignedUserId" class="form-select">
                                        <option value="">-- Select user --</option>
                                        @foreach ($employees as $employee)
                                            <option value="{{ $employee->id }}">{{ $employee->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('assignedUserId') <span class="text-danger small">{{ $message }}</span> @enderror
                                </div>
                            @endif
