...
class Step2 extends Component
{
    ...
    protected function loadWeekSlots(): void
    {
        ...
        $companyHours = $this->company->getCompanyHours();
        $now = now()->setTimezone('Europe/Warsaw');
        ...
        for ($i = 0; $i < 7; $i++) 
        {
            ...
            if ($isPast || !$dayKey || ($companyHours[$dayKey]['closed'] ?? false)) {
                $this->weekSlots[$date->format('Y-m-d')] = [];
                continue;
            }

            $slots = [];
            $openTime = Carbon::parse($companyHours[$dayKey]['open']);
            $closeTime = Carbon::parse($companyHours[$dayKey]['close']);
    ...
    {
        return view('livewire.company.booking.step2')
